...
double CompareArea(const double Circumscribed_rectangleArea , const double ContoursArea)
{
	double Area_difference = Circumscribed_rectangleArea - ContoursArea ;
...
inline NEOLIX_STATUS_LIST  getMaxContours(cv::Mat binaryImage, cv::vector<cv::vector<cv::Point>> &contours,int &maxContourId)
{
...
/**
* author: PengCheng,pengcheng@neolix.cn
* funtion:计算轮廓的长宽以及矩形的顶点(重构PopRang函数）
*/
...
{
    cv::Mat destImage;
    cv::Vector<cv::Point> maxcomtour;
    cv::vector<cv::vector<cv::Point>> contours;
    int maxContourId;
    cv::RotatedRect rectangle;
    ///获得分割
    segmentationImage(srcImage, destImage);
    ///获取最大轮廓
    NEOLIX_STATUS_LIST status =  getMaxContours(destImage,contours,maxContourId);
    if(NEOLIX_FALSE == status) return NEOLIX_FALSE;
    ///获取最大轮廓的最小外接矩形
    findMinRectangle(&contours[maxContourId], rectangle);
    ///计算外接矩形的像素长宽
    calPixelDistance(rectangle, PixXLenght, PixYLength);
    rectangle.points(point);
    ///计算外接矩形中不包括在轮廓的面积占有率
	double blank_area_rate = CompareArea(PixXLenght*PixYLength,cv::contourArea(contours[maxContourId]));
    if(blank_area_rate > 0.45)
	{
		std::cout << "---------------------" <<std::endl;
		std::cout << "    有遮挡物，拿开   " <<std::endl;
		std::cout << "---------------------" <<std::endl;
		PixYLength = PixYLength = 0;
	return NEOLIX_FALSE;
	}
	contuor.~Vector();
    contuor =contours[maxContourId];
    return NEOLIX_SUCCESS;
