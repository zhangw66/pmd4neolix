...
public:
    ...
    virtual void onMouseCallback(cv::Mat& img, int event, const cv::Point pnt)=0;

private:
    static void __onMouseCallback(int event, int x, int y, int flags, void* ustc);
    //static void __onMouseCallback(int event, int x, int y,  void* ustc);
...
{
public:
    static std::string depthStringAtLoc(const cv::Mat& img);
