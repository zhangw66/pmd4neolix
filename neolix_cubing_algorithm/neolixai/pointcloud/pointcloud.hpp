...
class pointcloud
{
    public:
        ...
        virtual void onlyModifySzie(unsigned long size_);
        /// use depth data as point clods
        int converToPointCloud(cv::Mat &pointClond );
        ///convert the depth data to point cloud
        int converToPointCloud(cv::Mat &depthImage ,double fu, double fv, double u0, double v0);
        ///use the point clouds
        int converToPointCloud(std::vector<Point3D<double>> &pointcloud);
        ...
        unsigned  rows;
        unsigned  cols;
        Point3D<Tp> *data;//Real data storage
        std::vector<Point3D<double> > points;//pretreatment point cloud,for filter out unqualified data,in our application we believe that data of depth zero is unquaified
...
template<typename Tp, unsigned int row, unsigned col > void  pointcloud<Tp,row,col>::onlyModifySzie(unsigned long size_)
...
template<typename Tp, unsigned int row, unsigned int col>  int  pointcloud<Tp,row,col>::converToPointCloud(cv::Mat &depthImage)
 {
 ...
 //for this funtion ,we consider that all pointcloud is quailified, if you want to use this method ,please pretreatment pointclouds
template<typename Tp, unsigned int row, unsigned int col>  int  pointcloud<Tp,row,col>::converToPointCloud(std::vector<Point3D<double>> &pointcloud)
{
    int size_ = pointcloud.size();
    if(size_ == 0) return NEOLIX_FALSE;

/*
    points.reserve(size_ + 1);
    points.clear();
*/
    this->resize(size_,size_);
    for(int i = 0; i < size_; i++)
    {
        this->data[i][0] = pointcloud[i][0];
        this->data[i][1] = pointcloud[i][1];
        this->data[i][2] = pointcloud[i][2];
    }
    return NEOLIX_SUCCESS;
}
