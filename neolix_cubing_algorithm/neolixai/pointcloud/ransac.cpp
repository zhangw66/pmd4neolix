...
ransac::ransac(pointcloud<double,1,1>* pts)
{
    this->ptcld = pts;
    this->model = nullptr;
    this->ForDepthData = true;
}

void ransac::isFitDepthData(bool isDepth)
{
    this->ForDepthData = isDepth;
}
ransac::~ransac()
{
     ...
     //   std::cout<<"===============find three rands points============="<<<<std::endl<<std::endl;
      ...
      //  std::cout<<pt1<<std::endl<<pt2<<std::endl<<std::endl<<std::endl;
        ...
        if( inner_iter <= 0)
       {
        ...
        ///for depth data fit plane
       if(this->ForDepthData)
       {
         int left_x = g_plane_rect.at<int>(0, 0);
         int left_y = g_plane_rect.at<int>(1, 0);

        L_Eq = (cv::Mat_<double>(3,3)<<( (*ptcld) [index[0]] )[0] + left_x,
                                       ( (*ptcld) [index[0]] )[1] + left_y, 1,

                                       ( (*ptcld) [index[1]] )[0] + left_x,
                                       ( (*ptcld) [index[1]] )[1] + left_y, 1,

                                       ( (*ptcld) [index[2]] )[0] + left_x,
                                       ( (*ptcld) [index[2]] )[1] + left_y, 1);

        R_Eq = (cv::Mat_<double>(3,1)<<( ((*ptcld)[index[0]])[2]),
                                       ( ((*ptcld)[index[1]])[2]),
                                       ( ((*ptcld)[index[2]])[2]));
       }else { //for point cloud fit plane

           L_Eq = (cv::Mat_<double>(3,3)<<( (*ptcld) [index[0]] )[0] ,
                                          ( (*ptcld) [index[0]] )[1] , 1,
